...
EOF
  ...
  # Verify Kiro fix added
  if [[ -f "$test_zshrc" ]]; then
    local content=$(cat "$test_zshrc")
    test_assert_contains "$content" "Restore Atuin keybindings" "Contains Kiro fix comment"
...
test_atuin_amazonq_backward_compat() {
  test_start "AC6: Amazon Q backward compatibility (deprecated function)"

  # The deprecated function should still work
  _atuin_configure_amazonq_compatibility >/dev/null 2>&1
  local result=$?

  test_assert_equals "$result" "0" "Legacy Amazon Q compatibility function still works"
  test_end
}
